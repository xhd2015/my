...
- Exit code `1`.
- Stderr contains `openclaw not found`, install instructions, and `Install openclaw now? [y/N]: `.

## Side Effects

- No `npm install` invocation recorded.
- No `openclaw gateway` invocation recorded.

## Errors

- OpenClaw CLI missing from `PATH`; user declined install prompt.
...
func Assert(t *testing.T, req *Request, resp *Response, err error) {
	...
	if !strings.Contains(resp.Stderr, "Install openclaw now? [y/N]: ") {
		t.Fatalf("stderr missing install prompt:\n%s", resp.Stderr)
	}
	if len(resp.NpmCalls) > 0 {
		t.Fatalf("npm should not be invoked:\n%v", resp.NpmCalls)
	}
