...
const openclawAgentDBMigrationSQL = `
...
func agentAuthDBPath(dataDir string) string {
	return filepath.Join(dataDir, "agents", "main", "agent", "openclaw-agent.sqlite")
}

func writeAuthProfileStore(dataDir string, store openclawAuthProfileStore) error {
	payload, err := json.Marshal(store)
	if err != nil {
		return err
	}

	dbPath := agentAuthDBPath(dataDir)
	...
	if _, err := db.Exec(openclawAgentDBMigrationSQL); err != nil {
	...
	if err != nil {
		return fmt.Errorf("write auth profile store: %w", err)
	}
	return nil
}